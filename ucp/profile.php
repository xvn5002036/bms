...
// *********************************   
?>
<b> --> 修改密碼　用戶ID:<?php echo $_SESSION['user_id']; ?></b><br />
<?php
	if (!isset($_POST['submit'])){
	//error handling
    if (isset($_GET['error'])){
    	  if (($_GET['error']) == "mmcdmin"){
        echo ("<font color=red>Error:Your new password is too Short. 6 characters Min!</br>出錯：您的用戶密碼是太短了。 小於6個位！</font><br />");}
        if (($_GET['error']) == "mmcdmax"){
        echo ("<font color=red>Error:Your new password is too long. 12 characters Max!</br>出錯：您的用戶密碼是太短了。 小於6個位！</font><br />");}
        if (($_GET['error']) == "wp"){
